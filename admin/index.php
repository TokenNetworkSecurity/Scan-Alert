<?php
	...
	$sql = 'SELECT * FROM `scams` WHERE `moderated` = 0 ORDER BY `id` DESC';
	...
	while ($report = mysqli_fetch_array($query)){
		...
		<tr>
			...
			<td class="data-actions"><button type="button" class="btn btn-success">Publish</button><button type="button" class="btn btn-danger">Delete</button><input type="hidden" value="' . $report['id'] . '"></td>
		</tr>';
	}
...
<body>
	<header>
		<ul class="p-0">
			<li><a href="/admin/" class="header__active-link">New Reports</a></li>
			<li><a href="/admin/published/">Published Reports</a></li>
		</ul>
	</header>
	<main>
	...
	<script>
		var id, address, scam_type, website, description, link, json;
